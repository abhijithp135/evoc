...
def build_candidates(current_graph, max_candidates, rng_state, n_threads):
    ...
    We will be assigning random weights to each of them and finally taking the neighbours
    with minimum distance as generated by random numbers.
    We will be resetting flag to 0 for those neighbours/indices of a given point which are
    new candidates for the same point
    ...
    for n in numba.prange(n_threads):
        ...
        for i in range(n_vertices):
            for j in range(n_neighbors):
                ...
                # At this point current_flags are newly inserted into indices
